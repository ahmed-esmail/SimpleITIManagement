...
public class StudentTempController : Controller
{
    ...
    {
        var dept = db.Departments.SingleOrDefault(dept => dept.DeptId == 200);
        var student = new Student {Name = "Ali", Age = 25, DeptNo = 400};
    ...
    {
        // eager load
        var std = db.Students.Include(std => std.Department).FirstOrDefault(student => student.Id == 1);
